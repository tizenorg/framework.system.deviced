...
static void power_supply_status_init(void)
{
	...
	/* do not update charge status.
	 * deviced don't know a currently status. */
...
static const struct edbus_method power_supply_edbus_methods[] = {
...
static void power_supply_init(void *data)
{
	...
	ret = register_edbus_method(DEVICED_PATH_SYSNOTI,
			power_supply_edbus_methods,
			ARRAY_SIZE(power_supply_edbus_methods));
	if (ret < 0)
