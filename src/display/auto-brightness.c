...
static bool alc_handler(void *data)
{
	...
	if (pm_cur_state != S_NORMAL || !hallic_open) {
		if (alc_timeout_id > 0)
			ecore_timer_del(alc_timeout_id);
		alc_timeout_id = NULL;
		return EINA_FALSE;
...
static bool check_sensor(void *data)
{
	...
	_I("change vconf value before registering sensor");
	return EINA_FALSE;
}
...
static int get_coord_supported(void)
{
	int ret;

	ret = dbus_method_sync(COORD_BUS_NAME,
	    COORD_PATH_AUTOBRIGHTNESS, COORD_INTERFACE_AUTOBRIGHTNESS,
	    METHOD_CHECK_SUPPORTED, NULL, NULL);

	_I("coord support state : %d", ret);
...
static int booting_done_cb(void *data)
{
	...
	if (get_coord_supported() == true)
		/* get level from coord (sensorhub-contextd) */
		prepare_level_handler(NULL);
	else
