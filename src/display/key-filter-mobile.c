...
#define KEY_MIN_CONTINUE_TIME		400	/* ms */
...
static inline int check_powerkey_continue(struct input_event *pinput)
{
	static long pre_msec;
	long msec;
	int diff;
	...
	msec = SEC_TO_MSEC((pinput->time).tv_sec) +
	    USEC_TO_MSEC((pinput->time).tv_usec);

	diff = msec - pre_msec;
	_D("pre_msec %d msec %d diff %d", pre_msec, msec, diff);

	pre_msec = msec;

	if (diff < KEY_MIN_CONTINUE_TIME)
		return 1;
...
static int process_power_key(struct input_event *pinput)
{
	...
	caps = display_get_caps(DISPLAY_ACTOR_POWER_KEY);
	block = check_powerkey_continue(pinput);
	...
	case KEY_PRESSED:
		...
		if (block == true) {
			ignore = true;
			skip_lcd_off = false;
		} else if (display_has_caps(caps, DISPLAY_CAPA_LCDON)) {
...
static void turnon_hardkey_backlight(void)
{
	...
	/* check key backlight is already on */
	if (ret < 0 || val)
		return;
	...
	if (ret < 0)
		_E("Fail to turn on key backlight!");
}
...
static int check_key_filter(void *data, int fd)
{
	...
	pinput = (struct input_event *)data;
	...
	case EV_KEY:
		...
		if (pinput->code == BTN_TOUCH) {
			if (!current_state_in_on())
				break;
			caps = display_get_caps(DISPLAY_ACTOR_TOUCH_KEY);
			if (!display_has_caps(caps,
...
static struct display_actor_ops display_touchkey_actor = {
	.id	= DISPLAY_ACTOR_TOUCH_KEY,
	.caps	= DISPLAY_CAPA_UPDATE_LCD_TIMEOUT,
};
...
static void keyfilter_init(void)
{
	...
	display_add_actor(&display_touchkey_actor);
