...
static int open_restricted(const char *path, int flags, void *user_data)
{
	...
	if (fd >= 0) {
		if (ioctl(fd, EVIOCSCLOCKID, &clockid) < 0)
			_E("Failed to change clock %s", path);
	}
...
static Ecore_Fd_Handler *add_poll_handler(struct libinput *li)
{
	int fd;
	Ecore_Fd_Handler *efd;
	...
	efd = ecore_main_fd_handler_add(fd, ECORE_FD_READ|ECORE_FD_ERROR,
		    input_handler, (void *)((intptr_t)li), NULL, NULL);
	if (!efd) {
		close(fd);
		return NULL;
	}
...
static int open_udev(void)
{
	...
	udev = udev_new();
	if (!udev) {
		_E("Failed to udev_new!");
		return -EIO;
	}

	li = libinput_udev_create_context(&interface, NULL, udev);
	if (!li) {
		_E("Failed to create udev context!");
		return -EIO;
	}

	ret = libinput_udev_assign_seat(li, SEAT_NAME);
	if (ret < 0) {
		_E("failed to assign seat");
		return -EIO;
	}

	/* add to poll handler */
	efd = add_poll_handler(li);
	if (!efd) {
		_E("Failed to add poll list");
		return -EIO;
	}
...
int init_input(int (*callback)(int data, PMMsg *msg))
{
	int ret = 0;
