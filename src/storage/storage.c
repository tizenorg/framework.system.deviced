...
static Ecore_Timer *mem_trim_timer;
...
static void storage_status_broadcast(struct storage_config_info *info, double total, double avail)
{
	...
	} else if (level <= info->critical_level) {
...
static Eina_Bool check_storage_status(void *data)
{
	...
	int type = 0;
	struct statvfs s;
	struct storage_config_info *info;
	double dAvail = 0.0;
	double dTotal = 0.0;
...
static int mem_trim_delta(struct tm *cur_tm)
{
	int delta = 0;
	int sign_val;
...
static int run_mem_trim(void)
{
	...
	mem_trim_time = MEM_TRIM_TIMER_INTERVAL + mem_trim_delta(cur_tm);
