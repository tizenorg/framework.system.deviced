...
static void hdmi_chgdet_cb(void *data)
{
	...
	if(val == 1) {
		...
		/* That 'old' variable has negative value means that this is
		 * the first update for HDMI status. In addition,
		 * if the HDMI status is 0 (disconnected) at the first update,
		 * it means that the device is turned on without HDMI cable.
		 * Thus the notification for disconnecting is ignored
		 */
		if (old >= 0)
