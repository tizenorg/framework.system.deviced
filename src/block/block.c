...
#define SDCARD_LINK_PATH    "*/sdcard/*"
...
static bool check_primary_partition(const char *devnode)
{
	...
	/* if no partition */
	if (!fnmatch(SDCARD_LINK_PATH, devnode, 0) ||
	    !fnmatch("*/mmcblk[1-9]", devnode, 0) ||
		...
		enum unmount_operation option)
{
	...
	/* Mobile specific:
	 * should unmount app2ext and ode whenever try to unmount */
...
static int block_init_from_udev_enumerate(void)
{
	...
	udev_list_entry_foreach(list_entry,
		...
		udev_list_entry_foreach(list_sub_entry,
			...
			if (!fnmatch(SDCARD_LINK_PATH, devlink, 0)) {
				devnode = devlink;
				break;
			}
		}
...
static void block_init(void *data)
{
	...
	register_notifier(DEVICE_NOTIFIER_EARLY_BOOTING_DONE, booting_done);
}
...
static void block_exit(void *data)
{
	...
	unregister_notifier(DEVICE_NOTIFIER_EARLY_BOOTING_DONE, booting_done);
